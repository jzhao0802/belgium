...
Run_BE_LR_Model<- function(all_samples_forsim, simulations,out_path, cutoff_score, Trial, logF){
	...
	run_in_par <- function(sim){
		...
		# validation_data<-all_samples_forsim[[i]][[2]]
		test_data<-all_samples_forsim[[sim]][[2]]
		...
		data <- as.data.frame(apply(training_data, 2, function(x){as.numeric(as.vector(x))}))
		mis_num <- apply(apply(corr_matrix, 1, is.na), 2, sum)
		get_corr <- function(thresh){
			corr_matrix_1 <- cor(data, use='complete.obs')
		...
		var_del2 <- as.vector(high_corr_result[high_corr_result[, 3]==1, 2])
		var_del_all<- c(var_del, var_del2) 
    ...
    temp_out <- sfClusterApplyLB(1:nrow(par_all_scenarios), run_Parallel)
	library(plyr)
	...
	execution_time<- (timeEnd-timeStart)[3]/60
	execution_time
	write.csv(saved_sim_outcome, 'model_result_for_LR.csv', row.names=F)
